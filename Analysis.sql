...
SELECT 
    ...
    MOD(ROUND(AVG(length)), 60) AS average_minutes
FROM film;
...
-- column called DAY_TYPE with values 'weekend' or 'workday', 
...
/*
You need to ensure that customers can easily access information about 
the movie collection. To achieve this, retrieve the film titles and their 
rental duration. If any rental duration value is NULL, replace it with the 
string 'Not Available'. 
Sort the results of the film title in ascending order.
Please note that even if there are currently no null values 
in the rental duration column, the query should still 
be written to handle such cases in the future.
...
SELECT 
	COUNT(DISTINCT film_id) as total_number_released
FROM film;

-- 1.2 The number of films for each rating.
SELECT 
    rating, 
    COUNT(*) AS films_for_rating
FROM 
    film
GROUP BY 
    rating;
...
GROUP BY 
    rating
ORDER BY films_for_rating DESC;
/*
